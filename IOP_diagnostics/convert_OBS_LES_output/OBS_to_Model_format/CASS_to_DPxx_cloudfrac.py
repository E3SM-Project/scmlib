...
input_file = "/global/homes/b/bogensch/THREAD/CASS_obs/cass_obs_cloud_fraction.nc"
output_file = "/pscratch/sd/b/bogensch/E3SM_simulations/iopdiags_OBS_and_LES_files/DP_EAMxx/CASS.obs.ARSCL.dpxx_format.nc"
time_offset = 0.0
...
time_data = ds_in["hour"].values
ds_out["time"] = xr.DataArray((time_data - 6.5)/24., dims=["time"])
...
three_d_vars = [
    ("cf", "cldfrac_tot_for_analysis_horiz_avg", 1.0),
]
...
#        data_val = np.squeeze(ds_in[var_in].values)[:,::-1]
        data_val = np.squeeze(ds_in[var_in].values).T[:, ::-1]
