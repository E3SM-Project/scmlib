...
input_file = "/global/homes/b/bogensch/THREAD/DP_paper_figures/ARM_data/LAFE/sgpdlprofwstats4newsC1.c1.20170823_24.000000.nc"
output_file = "/pscratch/sd/b/bogensch/E3SM_simulations/iopdiags_OBS_and_LES_files/DP_EAMxx/LAFE.obs.w2.dpxx_format.nc"
...
three_d_vars = [
    ("w_variance", "w_variance_horiz_avg", 1.0),
]
...
for var_in, var_out, factor in three_d_vars:
    if var_in in ds_in:
        ...
        if var_in == "w_variance":
            # Read and flip the 'noise' variable to match w_variance shape
            noise = np.squeeze(ds_in["noise"].values)[:, ::-1]
            data_val[noise > 5] = np.nan  # Apply the filtering	
